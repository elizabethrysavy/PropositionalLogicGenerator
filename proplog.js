...
// set by solver as it starts: used for printing to html
    ...
    var start_time=0;
    var solvable = false; //addition -ER
...
// ====== top level: calling and printing from/to html =====
    ...
    */
    ...
    function solve(txt) {
        var solver_algorithm = "dpll_better"; //addition -ER
        var trace_method = "none"; //addition - ER
        solve_aux(txt, solver_algorithm, trace_method);
        if(solvable == true) {
            return "Valid";
        }
        else {
    ...
    function solve_aux(txt,solver_algorithm,trace_method) {
        ...
        origvars=[];
        // formula
        converted=formula_to_cnf(parsed);
        converted=rename_vars_in_clauses(converted);
        maxvar=converted[0];
        clauses=converted[1];
        ...
        res=dpll(clauses,maxvar,trace_method,origvars);


        if (res[0]!==false) {
            solvable = true; //addition - ER

        } else {
            // clause set unsatisfiable
            solvable = false; //addition - ER
        }
    }
...
function dpll(clauses,maxvarnr,trace,varnames) {
